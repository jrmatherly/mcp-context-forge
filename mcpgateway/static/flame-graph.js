...
// biome-ignore-all lint/correctness/noUnusedVariables: used by HTML templates
class FlameGraph {
  ...
  renderNode(node, x, y, width, parentDuration = null) {
    ...
    const isSearchMatch = this.searchTerm && node.name.toLowerCase().includes(this.searchTerm.toLowerCase());
    ...
    if (node.children && node.children.length > 0) {
      ...
      node.children.forEach((child) => {
        ...
        svg += this.renderNode(child, childX, y + this.cellHeight, childWidth, duration);
...
// biome-ignore lint/style/useConst: reassigned from HTML template
let flameGraph = null;
