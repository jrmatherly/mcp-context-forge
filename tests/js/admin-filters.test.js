...
function buildTable(tbodyId, rows, { testIdAttr, startCol = 0 } = {}) {
  ...
  rows.forEach((cells) => {
    ...
    if (testIdAttr) {
      tr.setAttribute("data-testid", testIdAttr);
    }
    cells.forEach((text) => {
...
describe("filterServerTable", () => {
  ...
  test("case-insensitive search", () => {
    const tbody = buildServerTable([["act", "ico", "1", "uuid-1", "MyServer", "test"]]);
    f()("MYSERVER");
...
describe("filterEntitiesByTags", () => {
  ...
  test("whitespace-only filter shows all rows", () => {
    const tbody = buildTaggedTable("tools", [{ text: "Tool A", tags: ["production"] }]);
    f()("tools", "  ,  , ");
    const rows = tbody.querySelectorAll("tr");
    expect(rows[0].style.display).toBe("");
  });
