...
describe("showUserEditModal", () => {
  test("loads user edit form content via authenticated fetch fallback", async () => {
    ...
    expect(win.fetchWithAuth).toHaveBeenCalledWith("/admin/users/user%40example.com/edit", { method: "GET" });
  ...
  test("prefers HTMX loader when available", async () => {
    ...
    expect(win.htmx.ajax).toHaveBeenCalledWith("GET", "/admin/users/user%40example.com/edit", {
