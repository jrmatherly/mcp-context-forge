...
describe("generateUserId", () => {
  ...
  test("generates unique ID when no authenticated user", () => {
    ...
    try {
      win.sessionStorage.removeItem("llm_chat_user_id");
    } catch (e) {
      /* noop */
    }
    const result = f()();
...
describe("generateConfig", () => {
  ...
  test("generates stdio config", () => {
    ...
    expect(result.mcpServers["mcpgateway-wrapper"].env.MCP_SERVER_URL).toContain("/servers/s1");
  });
  ...
  test("cleans server name for config key", () => {
    const result = f()({ id: "s1", name: "My Special Server!" }, "sse");
    const key = Object.keys(result.servers)[0];
    expect(key).toBe("my-special-server");
  });

  test("throws for unknown config type", () => {
    expect(() => f()({ id: "s1", name: "x" }, "unknown")).toThrow(/Unknown config type/);
  });
});
