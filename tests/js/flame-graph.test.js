...
const srcPath = path.resolve(__dirname, "../../mcpgateway/static/flame-graph.js");
...
beforeAll(() => {
  ...
  dom = new JSDOM('<!DOCTYPE html><html><body><div id="flame-container"></div></body></html>', {
    url: "http://localhost",
...
describe("FlameGraph.getSpanColor", () => {
  test("returns red for error status", () => {
    const fg = new FlameGraph("flame-container", sampleTrace);
    expect(fg.getSpanColor({ status: "error", kind: "client" })).toBe("#ef4444");
  });
